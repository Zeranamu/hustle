...
func _tick():
	...
	#Clashing 1 2 3
		if current_tick == 11*3 or current_tick == 14*3 or current_tick == 17*3:
			...
			if mastersfx:
				...
				pass
			host.spawn_particle_effect(preload("res://_LamTyrant/characters/Assets/Fx/TyHitFx1.tscn"), Vector2(0,opos.y-18), Vector2(1, 0))
			pass
			if pos.x > opos.x:
	...
	#Clashing Final
		if current_tick == 22*3:
			...
			if mastersfx:
				...
				pass
			host.spawn_particle_effect(preload("res://_LamTyrant/characters/Assets/Fx/TyHitFx2.tscn"), Vector2(0,opos.y-18), Vector2(1, 0))
			host.spawn_particle_effect(preload("res://_LamTyrant/characters/Assets/Fx/TyShw1Fx.tscn"), Vector2(0,opos.y-18), Vector2(1, 0))
	...
	if name == "CrossIntro2":
		...
		if (current_tick == 390 && pos.x < opos.x):
			...
			return "Wait"
	#Dialog storage 
		if host.charname == "Cress":
			if host.opponent.charname == "Cress":
				$"%Dia1".text = "What kind of joke is this."
				$"%Dia2".text = "Took the words out of my mouth."
				$"%Dia3".text = "What imbecile would impersonate a King!"
			elif host.opponent.charname == "Mayu":
				$"%Dia1".text = "You filthy voidfoxes still exist!?"
				$"%Dia2".text = "You filthy voidfoxes still exist!?"
				$"%Dia3".text = "These maggots all think they can take me down."
			elif host.opponent.charname == "Blate":
				$"%Dia1".text = "You filthy voidfoxes still exist!?"
				$"%Dia2".text = "You were my son with that voidfox... It can't be."
				$"%Dia3".text = "You were my son with that voidfox... It can't be."
			elif host.opponent.charname == "Miko" or host.opponent.charname == "MikoEX":
				$"%Dia1".text = "You filthy voidfoxes still exist!?"
				$"%Dia2".text = "It seems your kind\ncan't even speak coherently."
				$"%Dia3".text = "It seems your kind\ncan't even speak coherently."
			elif host.opponent.charname == "Nolan":
				$"%Dia1".text = "What is this... You hamsters normally\ncan't be this strong."
				$"%Dia2".text = "Your kind was weak, mellow... Inferior."
				$"%Dia3".text = "They all need a reminder of their lowly place..."
			elif host.opponent.charname == "Noland":
				$"%Dia1".text = "A hamster who can read? That is just hilarious."
				$"%Dia2".text = "Reading? Don't make me laugh.\nYour kind was all mindless slaves."
				$"%Dia3".text = "They all need a reminder of their lowly place..."
			elif host.opponent.charname == "Syrie":
				$"%Dia1".text = "Surprised to see me, daughter?"
				$"%Dia2".text = "'Hitler'? You forgot the\nfather you killed already?"
				$"%Dia3".text = "The act of ripping my head of shall be repaid."
			elif host.opponent.charname == "Afro":
				$"%Dia1".text = "What business you have with me, jester?"
				$"%Dia2".text = "A manner befit a king yourself.\nTis a weird crown you don."
				$"%Dia3".text = "One can't be a both a clown\nand a jester. Or so I thought."
			elif host.opponent.charname == "Chimamire":
				$"%Dia1".text = "Hah! You, I like you." 
				$"%Dia2".text = "Funny, I don't see you being a saint either." 
				$"%Dia3".text = "Hmp, you are worthy of a cold welcome;\nof Death."
			elif host.opponent.charname == "Sculla":
				$"%Dia1".text = "A low-life touting as a Chef." 
				$"%Dia2".text = "Did I gave you permission to speak? Rodent."
				$"%Dia3".text = "Did I gave you permission to speak? Rodent."
			else:
				#neutralspeech
				$"%Dia1".text = "Kneel down, you stand before a king"
				$"%Dia2".text = "Not lowering your head?\nNow it shall be on the ground."
				$"%Dia3".text = "Not lowering your head?\nNow it shall be on the ground."
